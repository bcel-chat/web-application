            ...
            <div class="col-12 col-sm-6 col-md-6 col-lg-4" v-for="(history, index) in customer_requirement_history" :key="index">
                <div class="fi-card text-left">
                    <div class="fi-card-title">
                        ...
                        <span class="editIcon" v-if="getShowEdit(history['editCount'])"  @click="openEditDetail(history)">
...
export default {
  ...
  methods: {
    ...
    getAllPendingSearch(){
        ds.rpc.make('/bcel/api/flexible/interest/customer/requirement/by/staff/pending/search', { staffUser:  this.$store.getters['flexible_interest_module/user'], searchData: this.search_data.trim()}, ( error, result ) => {
            if(error){
                this.aclasses = [];
            } else{
    ...
    getAllPending(){
        ds.rpc.make('/bcel/api/flexible/interest/customer/requirement/by/staff/pending', { staffUser:  this.$store.getters['flexible_interest_module/user']}, ( error, result ) => {
            if(error){
                this.aclasses = [];
            } else{
                this.customer_requirement_history = result;
    ...
    openEditDetail(history){
        ...
        console.log(history);
        this.$store.commit('flexible_interest_module/addRequestApproveDetail', { info: history });
        this.$router.push({path: sessionStorage.getItem('flexible_root_router')+'/flexible/customer/requirement/edit/' + btoa(history['requirementId'])});
    },
