        ...
        <div class="container-fluid" id="fi_print">
            <div class="row">
                ...
                <div class="col-12 col-sm-12 col-md-12">
                    <h3 class="fi-title text-center"><strong>ລາຍການທີ່ໄດ້ຮັບການອະນຸມັດທັງໝົດ</strong></h3>
                </div>
                <div class="col-12 col-sm-12 col-md-12">
                        ...
                        <table class="table text-left">
                            ...
                            <tbody>
                                <tr v-for="(requirement, index) in customer_requirments" :key="index">
                                    ...
                                    <td><span v-if="(requirement['approveStatus'] == 'APPROVED')" :class="requirement['approveStatus']">&#10003;</span> <span  v-if="(requirement['approveStatus'] == 'CANCLE')" :class="requirement['approveStatus']">&#215;</span></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    methods: {
        ...
        dateValueChange(){
            if(this.begin_date && this.end_date){
                if((this.begin_date.getTime() > this.end_date.getTime())){
        ...
        getData(){
            ds.rpc.make('/bcel/chat/api/flexible/interest/report/customer/requirement/approved/all', { 
               limited:  this.limit, 
               beginDate: this.begin_date?this.begin_date.getTime():'', 
               endDate: this.end_date?this.end_date.getTime():'',
        ...
        downloadToExcel() {
            if(this.customer_requirments.length>0){
                import('@/vendor/Export2Excel').then(excel => {
                    const tHeader = ['ລະຫັດ', 'ຊື່ລູກຄ້າ', 'ອາຊີບ', 'ປະເພດ', 'ເບີໂທ', 'ຂໍ້ມູນອື່ນໆ', 'ຈຳນວນເງິນຝາກ', 'ສະກຸນເງິນ', 'ລະຫັດສະກຸນເງິນ', 'ດອກເບ້ຍທີ່ຕ້ອງການ', 'ໄລຍະຝາກ', 'ດອກເບ້ຍປົກກະຕິ', 'ດອກເບ້ຍຍຶດຍຸ່ນສູງສຸດ', 'ໄລຍະຮັບດອກເບ້ຍ', 'ຜູ້ຂໍອະນຸມັດ', 'ຂະແໜງ', 'ພະແນກ', 'ສາຂາ', 'ຄຳຄີດເຫັນຂອງພະນັກງານ', 'ວັນທີ່', 'ເວລາ', 'ຜູ້ອະນຸມັດ', 'ວັນທີ່', 'ເວລາ', 'ສະຖານະ', 'ໝາຍເຫດ']
                    ...
                    excel.export_json_to_excel({
                        header: tHeader,
                        data,
                        filename: 'report-interest-flexible-approved-all'+((this.begin_date && this.end_date)?'-date-'+this.getDate(this.begin_date).replace(/\//g,'-')+'-to-'+this.getDate(this.end_date).replace(/\//g,'-'):'')+'-limit-'+this.limit,
                        autoWidth: true,
                        bookType: 'xlsx'
                    });
                });
        ...
        getDate(date){
            if(date){
                var d = (date.getDate().toString().length>1)?date.getDate().toString():"0"+date.getDate().toString();
