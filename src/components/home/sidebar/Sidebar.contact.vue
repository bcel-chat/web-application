            ...
            <div class="grc" :key="index">
                ...
                <div class="ctnlb">
                    <template v-for="(detail, index) in contact">
                        <div class="ctl" :key="index" @click="getContact(detail)">
                            <div class="_2umId">
...
export default {
    ...
    computed: {
        ...
        contacts() {
            let i = this.$store.state.userDetail.contact.filter((item) => {
                let i = 1
    ...
    methods: {
        ...
        getTyping(data) {
            if(data){
                if(data.sender != this.$store.state.uinfo.uinfo.uid){
                    this.$store.dispatch('chats/typing', data.type)
                    localStorage.setItem('partner', data.sender)
                }
            }
        },
        getMsg(rid){
        ...
        newChat(e){
            if (!e){
                ds.rpc.make('setNewchat', {
                    rid: 0,
                    rname: '',
                    img: 'no',
                    lastseen: this.moment(new Date).format('YYYY-MM-DD'),
                    type: 1,
                    active: 0,
                    sender: this.$store.state.uinfo.uinfo.uid,
                    reciever: this.$store.state.chats.reciever.user_id,
                    status: 1
                }, (err,res)=>{
                    this.$store.dispatch('chats/roomcheck', res)
                    ds.event.subscribe(`message/${res}`, data => {
                    ...
                    this.record.subscribe(`online/${this.$store.state.chats.reciever.user_id}`, data => {
                        this.$store.dispatch('chats/online', data)
                    }, true)
                })
            }else{
                this.getMsg(e)
